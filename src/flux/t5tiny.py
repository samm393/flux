...
class NewGELUActivation():
    ...
    def __call__(self, input: Tensor) -> Tensor:
        return 0.5 * input * (1.0 + Tensor.tanh(math.sqrt(2.0 / math.pi) * (input + 0.044715 * Tensor.pow(input, 3.0))))
...
class T5Attention():
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        relative_position_if_large = Tensor.min(
            Tensor.stack(relative_position_if_large, Tensor.full_like(relative_position_if_large, num_buckets - 1)),
            axis=0
        )
    ...
    def __call__(
        ...
        key_states = project(
            hidden_states, self.k,
        )
        value_states = project(
            hidden_states, self.v,
        )
